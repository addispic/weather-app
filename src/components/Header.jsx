...
const Header = () => {
  ...
  // profile navigator handler
  const profileNavigatorHandler = (index) => {
    if (index > 0) {
    ...
    <header className="bg-white border-b border-neutral-300 fixed left-0 top-0 w-full z-50">
      ...
      <div className="max-w-[1524px] mx-auto px-[3%] flex items-center justify-between">
        ...
        <div>
          {/* site logo */}
          <NavLink to={"/"}>
            <p className="lg:text-lg py-1 font-medium text-green-600 transition-colors ease-in-out duration-150 hover:text-green-500">
              w<span className="hidden sm:inline-block">eather</span><span className="font-black uppercase">Forecast</span>
            </p>
          </NavLink>
        </div>
        {/* right section */}
        <div className="flex items-center justify-between gap-x-3 lg:gap-x-7">
          {/* links */}
          <div className="flex items-center justify-center gap-x-3 lg:gap-x-5">
            {/* home link */}
            <NavLink
              to={"/"}
              className={`relative flex items-center justify-center gap-1.5 after:absolute after:left-0 after:bottom-0  after:h-[2px] after:bg-green-600 after:transition-all after:ease-in-out after:duration-150  ${
                currentPathname === "/"
                  ? "after:w-full text-green-700"
                  : "after:w-0 hover:after:w-full text-neutral-500"
              }`}
            >
              {/* text */}
              <span>Home</span>
            ...
            {user ? (
              ...
              <NavLink
                to={"/login"}
                className={
                  "px-3 py-0.5 rounded-full transition-colors ease-in-out duration-150 hover:bg-green-500 bg-green-600 text-white text-sm"
                }
              >
                Login
              </NavLink>
          ...
          {user ? (
            <div className="relative">
              ...
              >
                <span className="text-sm text-green-700 hidden md:inline-block">{user?.username}</span>
                ...
                <MdKeyboardArrowDown
                  className={`text-green-600 text-lg transition-transform ease-in-out duration-150 ${
                    isProfileOn ? "-rotate-180" : "rotate-0"
                  }`}
                />
              </div>
              {/* pop up */}
              <div
                className={`absolute right-0 bg-transparent top-[2rem] overflow-hidden transition-all ease-in-out duration-300 ${
                  isProfileOn ? "h-[35rem]" : "h-0"
                }`}
              >
                {/* content */}
                <div className="bg-white rounded-md  border border-neutral-200 shadow-lg p-5 relative mt-3 mx-3">
                  ...
                  <div>
                    ...
                    <div className="mx-auto w-[150px] aspect-square bg-green-600 relative overflow-hidden rounded-md">
                      ...
                      {profiles?.length > 0 && (
                        <div className="absolute top-0 right-0">
                          <button
                            className="w-[20px] aspect-square rounded-full flex items-center justify-center overflow-hidden bg-white text-red-500"
                            onClick={() => {
                              deleteProfileHandler(profiles[currentIndex]._id);
                            }}
                          >
                            <RiDeleteBinLine />
                          </button>
                    ...
                    <div className="mt-1.5 pt-3 flex flex-col items-center justify-center">
                      ...
                      <button
                        className="w-full border border-green-600 text-sm mt-2.5 py-0.5 rounded-sm flex items-center justify-between px-1.5 text-green-600 transition-colors ease-in-out duration-150 hover:bg-green-600 hover:text-white"
                        onClick={() => {
